...
function gerarPaginaDeFechamento(authToken, dadosFiltrados, nomeFuncionarioSelecionado, nomeCompradorSelecionado, tipoDeVisualizacaoAtual, nomeCliente) {
  ...
  if (nomeFuncionarioSelecionado === 'TODOS' && nomeCompradorSelecionado !== 'TODOS') {
    ...
    linhasTabela = dadosDeVendas.map(item => {
      ...
      return `<tr><td>${dataFormatada}</td><td>${item.quantidade || ''}</td><td>${item.produto || ''}</td><td>${(Number(item.valorUnitario) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td><td style="color:green;">${(Number(item.valorTotal) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td><td>${item.funcionario || ''}</td></tr>`;
    }).join('');
  } else {
    tituloRelatorio = 'Relatório de Fechamento - ' + (nomeFuncionarioSelecionado === 'TODOS' ? 'Geral' : nomeFuncionarioSelecionado);
    const cabecalhoFuncionario = nomeFuncionarioSelecionado === 'TODOS' ? '<th>Funcionário</th>' : '';
    const celulaFuncionario = (item) => nomeFuncionarioSelecionado === 'TODOS' ? `<td>${item.funcionario || ''}</td>` : '';
